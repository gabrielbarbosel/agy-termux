#!/usr/bin/env bash
# agy-termux installer — Complete setup: downloads AGY, patches, configures shell.
# Usage: curl -fsSL https://raw.githubusercontent.com/gabrielbarbosel/agy-termux/main/install.sh | bash
set -euo pipefail

REPO="https://raw.githubusercontent.com/gabrielbarbosel/agy-termux/main"
BASE="$HOME/.local/share/agy-termux"
BIN_DIR="$HOME/.local/bin"
GLIBC="${PREFIX:-/data/data/com.termux/files/usr}/glibc/lib"

echo "agy-termux — Antigravity CLI for Android"
echo ""
...
if [ -f "$SCRIPT_DIR/adapter/wrapper.sh" ]; then
  ...
  for f in wrapper.sh update.sh self-heal.sh dns-heal.js patch; do
...
sed "1s|#!/usr/bin/env sh|#!${PREFIX:-/data/data/com.termux/files/usr}/bin/sh|" \
  "$BASE/adapter/wrapper.sh" > "$BIN_DIR/agy"
chmod +x "$BIN_DIR/agy"

# Download + patch AGY binary (reuses update.sh)
bash "$BASE/adapter/update.sh"

# Shell config
RC="$HOME/.bashrc"
if ! grep -q 'agy-termux/adapter/self-heal.sh' "$RC" 2>/dev/null; then
  cat >> "$RC" << 'EOF'
export PATH="$HOME/.local/bin:$PATH"
agy() {
  hash -r
  "$HOME/.local/share/agy-termux/adapter/self-heal.sh"
  "$HOME/.local/bin/agy" "$@"
}
EOF
...
echo ""
echo "✓ Installed. Run: source ~/.bashrc && agy"
